...
use serenity::{
    ...
    client::Context,
    futures::future, model::{guild::{Member, PartialGuild, Role}, misc::Mention, id::RoleId}, http::AttachmentType,
};
...
async fn download(
    ...
    let guild_id = match ctx.data.read().await.get::<data::ConfigValuesData>() {
        ...
        None => return LogError::new_slash("Guild configuration could not be loaded", aci.clone()).into(),
    };
    ...
    for t in dbtiers {
        ...
        for r in dr {
            match r {
                ...
                None => log.push(format!("Failed to find a discord role for tier: {}", t.name)),
            }
        }
    ...
    let msg = msg.channel_id.send_message(ctx, |m| {
        m.embed( |e| {
            ...
            e.field(
                "Details",
                format!("Format: {}\nCreated: <t:{}>", data.output, data.created.timestamp()),
                false
            );
            e.field(
                "Trainings",
    ...
    aci.edit_original_interaction_response(ctx, |r| {
        r.content(format!("[Done]({})", msg.link()))
    }).await?;
